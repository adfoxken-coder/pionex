...
DEFAULT_CONFIG = {
    "kline_interval": "5M",          # 使用的 K 線週期,需與排程頻率搭配
...
def get_perp_symbols():
    """取得目前 Pionex 支援的所有合約(PERP)幣種,回傳 {symbol: baseCurrency}"""
    resp = requests.get(
    ...
    return {
        s["symbol"]: s.get("baseCurrency", s["symbol"])
...
def main():
    ...
    symbols_map = get_perp_symbols()      # {symbol: baseCurrency}
    tickers = get_perp_tickers()          # {symbol: ticker}

    # 條件一:先用 24 小時成交金額篩選,減少後續 K 線 API 呼叫量
    candidates = []
    for symbol, base_currency in symbols_map.items():
        ...
        if amount_24h > config["min_24h_amount_usdt"]:
            candidates.append((symbol, base_currency))

    print(f"通過 24 小時成交金額篩選的幣種數量:{len(candidates)} / {len(symbols_map)}")
