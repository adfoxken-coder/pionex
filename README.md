...
台北時間每天 06:00–23:59,每 15 分鐘檢查一次,判斷同一套技術條件是否符合。
**依照「1小時 / 4小時 / 日線」哪個週期真的有新K線收盤,決定這次要不要偵測**
(見下方「多週期分層偵測」),符合就透過 Telegram 推播訊息通知你,並標出
哪些幣種在多個週期「共振」。完全免費,不需要自己的電腦或伺服器(repo 目前是
Public,GitHub Actions 執行時間無上限)。
...
- 執行環境:**GitHub Actions**,台北時間每天 06:00–23:59、每 15 分鐘檢查一次
- 通知方式:**Telegram Bot**,發送訊息給你自己

## 多週期分層偵測(1 小時 / 4 小時 / 日線)

**不是每 15 分鐘都會發訊息**,而是每次執行時直接問 Pionex:「1小時 / 4小時 /
日線 K 線,最新收盤的那一根,是不是比上次記錄的更新」:

- **1小時K線比上次記錄的更新**(代表確實有新的一根1小時K線收盤了):
  偵測 **1 小時級別**
- **4小時K線比上次記錄的更新**:偵測 **4 小時級別**
- **日線K線比上次記錄的更新**:偵測 **日線級別**
- **三個週期都沒有新K線收盤**:這次完全不用做事,直接跳過(節省 API 呼叫),
  也不會發送任何訊息

因為最細的週期是 1 小時,大部分的 15 分鐘檢查(每小時裡的另外 3 次)都會
是「沒有新K線收盤,直接跳過」,實際真正抓資料判斷大約每小時一次(遇到
4小時、日線收線時再加碼判斷)。

每次判斷完後,會把「這次看到的最新1小時/4小時/日線K線時間」記錄到
`state.json`,下次執行時拿來比對。

> **為什麼不用「現在幾點幾分」來判斷?** 因為 GitHub Actions 的排程實際觸發
> 時間常常會延遲 5~10 分鐘甚至更多(這是 GitHub 官方本身的限制,不是我們
> 能控制的),用時鐘猜測「現在是不是整點」很容易因為排程延遲而誤判、漏掉該
> 發送的通知。改成直接問交易所「有沒有新K線收盤」,不管排程延遲多久,只要
> 真的有新的一根收盤,下次執行時一定會抓到,不會漏掉,也不會因為同一根K線
> 期間內多跑了幾次而重複觸發。

訊息內容會依照這次實際偵測了哪些週期,只顯示對應的區塊;**共振區塊只在這次
同時偵測了 2 個以上週期時才會出現**(也就是剛好有兩個以上週期同時收線時,
例如 4 小時收線的同時也剛好整點)。

## 修正:避免抓到「還沒真正收盤」的K線

早期版本發現一個問題:排程觸發、程式立刻去抓資料時,交易所那邊剛收盤的
K線可能還沒完全寫入結算好,導致抓到的其實是「上一根」舊資料,判斷出來的
漲跌幅會慢半根K線,资讯有延遲。

修正方式:**程式一開始會先等待 `settle_delay_sec`(預設 45 秒)才真正開始抓資料**,
確保交易所那邊該收盤的K線都已經確實寫入完成。

## 偵測條件(可在 `config.json` 調整數值)

**同一套條件,在該次被偵測到的週期(1小時/4小時/日線)上各自獨立判斷**。
...
6. **額外規則(只套用在 1 小時級別)**:漲幅需 ≥ `min_pct_change_1h`(預設 1%)
   才推播;4小時、日線級別不受這條規則影響
...
排程還是每 15 分鐘觸發一次(用來檢查有沒有新K線收盤),但不代表每次都會
真正抓資料判斷或發送通知,詳見上方「多週期分層偵測」。
...
> **與型態訊號監控(pattern_alert.py)共用排除清單:** 這三份清單現在存放在
> `shared_excluded_assets.json`(位於這個 pionex repo 根目錄),兩支程式
> 每次執行都會即時從這裡抓取最新清單,只要更新這一個檔案,兩邊下次執行
> 就會自動套用,不用兩邊各改一次。如果抓取失敗(網路問題或檔案被刪除),
> 會自動退回使用各自 `config.json`/`pattern_config.json` 裡的本地備援清單。
...
**一般 15 分鐘檢查(同一小時內、還沒新K線收盤)——不會發送任何訊息**

**整點,1小時K線收盤——只有 1 小時級別:**
...
**4 小時收線時間點(例如 08:00)——1 小時 + 4 小時級別,並附共振:**
...
**日線收線時間點(例如台北時間 08:00,剛好也是整點跟4小時收線)——1 小時 + 4 小時 + 日線,完整共振:**
...
- **每個週期各自的區塊只在「該週期有符合條件的幣種」時才出現**,這次該偵測的
  週期都沒有符合就整則訊息都不發送;三個週期都沒有新K線收盤時,完全不會執行
  偵測、也不會發送訊息
- **共振區塊**:只在這次同時偵測了 2 個以上週期時才可能出現,同一幣種若同時
  出現在 2 個以上週期,會列出是哪些週期(`1h`/`4h`/`1d`);只出現在單一週期
  的幣種不會列入共振
...
- `min_pct_change_1h`:額外規則,只套用在 1 小時級別,漲幅需 ≥ 這個百分比才推播
  (預設 1.0,代表 1%);4小時、日線不受影響
...
- `excluded_base_currencies`、`excluded_stablecoin_bases`、`exclude_name_keywords`:
  ...
  (現在跟型態訊號監控共用 `shared_excluded_assets.json`,這三個欄位只在
  共用檔案抓取失敗時當備援使用)
...
| `state.json` | 五條件監控自動維護的執行紀錄,包含用來判斷1小時/4小時K線是否有新一根收盤的邊界時間,不需要手動編輯 |
